...
//		ipl.addImagesFromFolder("./images/document/"); // 10 images
		...
		LinkedList<Measure> MeasuresTemp = new LinkedList<>(measure_list.getMeasures());
		...
		while (m.getSize()+etalon>=etalon3) {
			Clusters.put(etalon3, new ArrayList<>());
			etalon3+=2;
		...
		for (Integer k:Clusters.keySet()) {
			for (Measure grain : MeasuresTemp) {
				if (grain.getSize() > (k-etalon)&&grain.getSize()<k) {
					Clusters.get(k).add(grain);
				}
				}
			MeasuresTemp.removeAll(Clusters.get(k));
		}
		for (Iterator<Integer> iterator = Clusters.keySet().iterator(); iterator.hasNext(); ) {
		    Integer value = iterator.next();
		    if (Clusters.get(value).isEmpty()) {
		        iterator.remove();
		    }
		}
		
		int size1=0;
		for ( Integer a: Clusters.keySet() ) {
	...
	*/
	...
	etalon1=0;
	 etalon2=2;
		...
		for ( Integer a: Clusters.keySet() ) {
			size1+=Clusters.get(a).size();		
			System.out.println(Clusters.get(a).size());}
